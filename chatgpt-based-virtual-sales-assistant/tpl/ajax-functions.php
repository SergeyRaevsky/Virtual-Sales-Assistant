...
function chatgpt_sales_manager_determine_name_language() {
    if (!isset($_POST['security']) || !wp_verify_nonce($_POST['security'], 'chatgpt_sales_manager_nonce')) {
        wp_send_json_error(['response' => __('Security check failed.', 'chatgpt-sales-manager')]);
    }

    $user_input = isset($_POST['user_input']) ? sanitize_text_field($_POST['user_input']) : '';

    if (empty($user_input)) {
        wp_send_json_error(['response' => __('User input is missing.', 'chatgpt-sales-manager')]);
    }
    ...
    if (empty($openaiApiKey)) {
        wp_send_json_error(['response' => __('Error: API key is missing. Please configure it in the plugin settings.', 'chatgpt-sales-manager')]);
    }
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode([
        'model' => $model,
        'messages' => [
            ['role' => 'system', 'content' => __('You are a helpful assistant.', 'chatgpt-sales-manager')],
            ['role' => 'user', 'content' => sprintf(__('Extract the name and preferred language from this text: "%s". Provide the result in the format \'Name: [name], Language: [language]\'. If the name is not clearly mentioned, just return \'User\'. The language should be in ISO 639-1 code.', 'chatgpt-sales-manager'), $user_input)]
        ],
        'max_tokens' => 50,
    ]));
    ...
    if (curl_errno($ch)) {
        wp_send_json_error(['response' => sprintf(__('Error: %s', 'chatgpt-sales-manager'), curl_error($ch))]);
    }
    curl_close($ch);

    $responseData = json_decode($response, true);
    if (isset($responseData['error'])) {
        wp_send_json_error(['response' => sprintf(__('API Error: %s', 'chatgpt-sales-manager'), $responseData['error']['message'])]);
    }
...
function chatgpt_sales_manager_handle() {
    ...
    if (empty($context)) {
        wp_send_json_error(['response' => __('Context is missing.', 'chatgpt-sales-manager')]);
    }
    ...
    $botResponse = $responseData['choices'][0]['message']['content'] ?? __('No response', 'chatgpt-sales-manager');
